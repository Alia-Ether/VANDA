...
@loader.tds
class VANDASettingsMod(loader.Module):
    """Advanced settings for VANDA Userbot"""

    strings = {"name": "VANDASettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"vanda-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "vanda" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_vanda(self, message: Message):
