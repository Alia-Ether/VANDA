...
async def asset_channel(
    ...
    *,
    ...
    # legacy vanda / hikka chats conversion to vanda
    if title.startswith("hikka-"):
        title = title.replace("hikka-", "vanda-")
...
def get_commit_url() -> str:
    """
    Get current VANDA git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return f'<a href="https://github.com/Alia-Ether/VANDA/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/Alia-Ether/VANDA/commit/{hash_}" if hash_ else "",
    )
