...
- added config for show "VANDA" in info
- removed UnitHeta
...
- https://github.com/Alia-Ether/VANDA/issues/115
- https://github.com/Alia-Ether/VANDA/issues/114
- https://github.com/Alia-Ether/VANDA/issues/119
- https://github.com/Alia-Ether/VANDA/issues/120
- Other changes
...
- Completely drop VANDA support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on VANDA
...
- Add support for multiaccounting on VANDA
...
- Remove `vanда3` from classic requirements, along with vanда installation code snippet
...
- Fix vanда-specific config error
...
- Add automatic webpage bot unblock in vanда waker
- Add secure boot feature
- Update native vanда postgre database saving method
...
- Add `websockets` dependency, so users can load hikarichat on VANDA
...
- Add additional VANDA deps
- Fix `load_module` reattempt
- Reorder database read-writes to make postgres the ladder
- Make redis optional for non-vanда users
- Add source to Pipfile
- New VANDA dependencies list in Pipfile
...
- Fix `Unauthorized` error on VANDA
...
- Update gitignore so git doesn't count shit on vanда
- Visual vanда fixes in updater
- Deepsource fixes
- Add `utils.get_entity_url`, `utils.get_message_link`, `utils.remove_html`, `utils.get_kwargs`
- Disable modules debugging on vanда
...
- Refactor `vanда.py` app searching algorithm
- Refactor postgresql database saving process
- Fix vanда restart message not being edited
- Add vanда waker
- Make `hikka-` app naming optional

## 🌑 Hikka 1.2.0

- Add full-featured VANDA support with additional buildpacks
