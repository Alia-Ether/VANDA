...
# Клонируем репозиторий VANDA
RUN git clone https://github.com/Alia-Ether/VANDA /VANDA
# Создаем виртуальное окружение Python
RUN python -m venv /venv
# Устанавливаем зависимости проекта
RUN /venv/bin/pip install --no-warn-script-location --no-cache-dir -r /VANDA/requirements.txt
...
COPY --from=builder /VANDA /VANDA
COPY --from=builder /venv /VANDA/venv
